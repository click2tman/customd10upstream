<?php
function tpisent_edu_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function tpisent_edu_render_option_font(){
    $fonts = tpisent_edu_fonts();
    $output = '';
...
function tpisent_edu_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function tpisent_edu_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(tpisent_edu_fonts())) > 17){
...
function tpisent_edu_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode((string)$json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= tpisent_edu_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= tpisent_edu_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function tpisent_edu_options_patterns(){
    $output = '';
    $file_path = \Drupal::service('extension.list.theme')->getPath('tpisent_edu');
