...
function tpisent_edu_check_registered_course($uid, $course_id){
...
function tpisent_edu_preprocess_course(&$vars){
  ...
  $uid = $current_user->id();
  if(tpisent_edu_check_registered_course($uid, $id)){
...
function tpisent_edu_preprocess_commerce_product__course(&$vars){
  $vars = tpisent_edu_preprocess_course($vars);
}
function tpisent_edu_preprocess_commerce_product__course__teaser(&$vars){
  $vars = tpisent_edu_preprocess_course($vars);
}
function tpisent_edu_preprocess_commerce_product__course__teaser_2(&$vars){
  $vars = tpisent_edu_preprocess_course($vars);
}
function tpisent_edu_preprocess_commerce_product__course__featured(&$vars){
  $vars = tpisent_edu_preprocess_course($vars);
}

function tpisent_edu_preprocess_node__lesson(&$variables) {
  $variables['#attached']['library'][] = 'tpisent_edu/gavias-lesson-video';
  ...
  if($node = $variables['node']){
    ...
    if(in_array("administrator", $role)){
      ...
      if($lesson_access == 'registered'){
        if( $user->id() && tpisent_edu_check_registered_course($user->id(), $course_id) ){
          $lesson_icon = 'fa fa-check';
          if(isset($variables['content']['field_lesson_content'])){
            $lesson_content = $variables['content']['field_lesson_content'];
          }    
